...
t2=datetime('now');
durByesOpt=t2-t1; % Duration of baysian opt.


%% 3. Random forest learning curves (number of trees optimization)

%Calculate Learning curves (training data), number of trees against training/validation error %
...
t2=datetime('now');
durLearnCurveRF=t2-t1; % Duration of getting learning curves
...
for learn_rate = gridLearn
    ...
    weight = logisticRegressionWeights( train2Data, train2Targ, w0, 1000, learn_rate); % Logistic regression spec
    [pred_probs,pred_classes] = logisticRegressionClassify( val2Data, weight ); % Predicted probabilities, and classes
...
%Calculate Learning curves (training data), number of iterations against training/validation error %
...
t2=datetime('now');
durLearnCurveLR=t2-t1; % Duration of logistic regression learning curve estimation
