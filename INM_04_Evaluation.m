...
valTarg_cat = categorical(valTarg); % Data prep
RF_predictions_cat=categorical(RF_predictions); % Data prep
...
LR_predictions_cat=categorical(LR_predictions); % Logistic regression predictions
...
for x = 1  : length(Bookmaker_prob_train)
   if Bookmaker_prob_train(x)>0.5 % Seperation condition
     Bookmaker_class_train(x) = 1; % Positive class
   else
     Bookmaker_class_train(x) = 0; % Negative class
   end
end

for x = 1  : length(Bookmaker_prob_test)
   if Bookmaker_prob_test(x)>0.5 % Seperation condition
     Bookmaker_class_test(x) = 1; % Positive class
   else
     Bookmaker_class_test(x) = 0; % Negative class
   end
end
